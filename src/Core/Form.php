...
use BIT\Core\FormElements\AbstractFormElement;
...
class Form
{
    ...
    public function isValid()
    {
        ...
        if (!$this->validateCsrf()) {
            App::getSession()->addFlash('Срабатывание защиты CSFR');
            return false;
        }
    ...
    private function validateCsrf()
    {
        ...
        if (null === ($csrf = App::getRequest()->post('__CSRF'))) {
            return false;
        }

        return App::getRequest()->validateCsrfToken($csrf);
    }
    ...
    {
        if ($this->action === null) {
            return '/' . Helper::camel2id(Helper::rTrimWord(App::getAction(), 'Action'));
        }
        return $this->action;
    }
